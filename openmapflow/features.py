...
from openmapflow.constants import FEATURE_FILENAME
...
def check_unused_features_df(df: pd.DataFrame, datasets: List[LabeledDataset]) -> str:
    """
    Remove features with no label
    """
    feature_name_list = []
    for d in datasets:
        feature_name_list += d.load_labels()[FEATURE_FILENAME].tolist()

    features_df_stems = df.filename.apply(lambda p: p.stem)
    features_with_no_label = df[~features_df_stems.isin(feature_name_list)]
    if len(features_with_no_label) > 0:
        prune_unused_features = input(
            f"Found {len(features_with_no_label)} features with no label, remove? [y]/n: "
        )
        if prune_unused_features.lower() == "n":
            return f"\u2716 Found {len(features_with_no_label)} features with no label"

        features_with_no_label[FEATURE_FILENAME].apply(lambda p: Path(p).unlink())
    return "\u2714 No features with no label found"
...
def create_features(datasets: List[LabeledDataset]):
    ...
    unused_text = check_unused_features_df(df, datasets)
    print(unused_text)
    report += (
        "\n\nAll data:\n" + empty_text + "\n" + duplicates_text + "\n" + unused_text
    )
